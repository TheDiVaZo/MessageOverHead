...
public class AdminCommands extends BaseCommand {
    ...
    public static void onCreate(Player player, BubbleGenerator bubbleGenerator, String message) {
        MessageOverHead.getBubbleManager().spawnBubble(message, bubbleGenerator, player, new HashSet<>(Bukkit.getOnlinePlayers()));
        //player.sendMessage(MessageOverHead.getConfigAdapter().getSettings().getCreateCommandMessage());
    }
    ...
    public static void onReload(CommandSender commandSender) {
        try {
            MessageOverHead.reloadConfigManager(true);
